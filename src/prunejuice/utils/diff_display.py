...
def display_worktree_status(status: Dict[str, Any]) -> None:
    ...
    if unstaged_files:
        unstaged_list = ", ".join(
            [f"{f['file']} ({f['status']})" for f in unstaged_files]
        )
...
def display_diff_with_pager(diff_text: str, title: str = "Git Diff") -> None:
    ...
    panel = Panel(formatted_diff, title=title, border_style="blue", expand=False)
...
def display_diff_error(error_msg: str) -> None:
    ...
    """
    panel = Panel(Text(error_msg, style="red"), title="Diff Error", border_style="red")
    console.print(panel)
