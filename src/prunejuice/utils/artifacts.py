...
class ArtifactStore:
    ...
    def create_session_dir(
        self, project_path: Path, session_id: str, command_name: str
    ) -> Path:
...
""")
    ...
    def store_file(
        self, session_dir: Path, file_path: Path, artifact_type: str = "output"
    ) -> Path:
    ...
    def get_session_artifacts(self, session_dir: Path) -> dict:
        ...
        for subdir in session_dir.iterdir():
            if subdir.is_dir() and subdir.name in [
                "logs",
                "outputs",
                "prompts",
                "specs",
