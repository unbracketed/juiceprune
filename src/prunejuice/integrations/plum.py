...
class PlumIntegration:
    ...
    def list_worktrees(self, project_path: Path) -> List[Dict[str, Any]]:
    ...
    def remove_worktree(self, project_path: Path, worktree_path: Path) -> bool:
        """Remove a worktree using native Git operations."""
        try:
            git_manager = GitWorktreeManager(project_path)
            success = git_manager.remove_worktree(worktree_path)
