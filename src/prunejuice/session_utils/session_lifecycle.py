...
class SessionLifecycleManager:
    ...
    async def create_session_for_worktree(
        ...
        try:
            ...
            if success:
                logger.info(f"Created session '{session_name}' for worktree {worktree_path}")
                return session_name
            else:
    ...
    async def cleanup_orphaned_sessions(
        ...
        try:
            ...
            if dry_run:
                logger.info(f"Dry run: Would clean {len(results['orphaned'])} orphaned sessions")
            else:
                logger.info(f"Cleanup complete: {results['total_cleaned']} cleaned, "
