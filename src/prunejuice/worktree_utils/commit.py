...
class CommitStatusAnalyzer:
    ...
    def _create_file_info(self, filepath: str, status: FileStatus, staged: bool) -> FileInfo:
    ...
    def _get_file_diff_stats(self, filepath: str, staged: bool) -> Dict[str, int]:
        ...
        try:
            if staged:
                ...
                diff_output = self.repo.git.diff("--cached", "--numstat", "--", filepath)
            else:
...
class InteractiveStaging:
    ...
    def get_file_diff(self, filepath: str, staged: bool = False, context_lines: int = 3) -> str:
        ...
        try:
            if staged:
                return self.repo.git.diff("--cached", f"-U{context_lines}", "--", filepath)
            else:
    ...
    async def execute_commit(
        ...
        try:
            ...
            if not allow_empty:
                ...
                staged_changes = [line for line in status.split('\n') if line and line[0] != ' ' and line[0] != '?']
